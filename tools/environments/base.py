...
#
# Kept in sync with gateway.session_context._VAR_MAP: the HERMES names share
# these prefixes, while TERMINAL_CWD is the one legacy unprefixed bridge.
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (HERMES_SESSION_|HERMES_UI_SESSION_ID|HERMES_CRON_AUTO_DELIVER_|TERMINAL_CWD)"
)
...
def _export_dump_excluding_session_vars(tmp_path: str) -> str:
    ...
    We drop the HERMES_SESSION_* / UI / CRON_AUTO_DELIVER lines and
    TERMINAL_CWD so they never persist across sessions in the shared snapshot.
    ``grep -vE`` returns exit 1
...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        #
        # The temp name MUST be unique per concurrent writer. macOS ships bash
        # 3.2 where ``$BASHPID`` is unset, and ``$$`` stays the parent shell PID
        # in ``&``-launched subshells. ``mktemp`` is portable on macOS/Linux and
        # creates a unique same-directory file for the atomic ``mv``.
        ...
        bootstrap = (
            ...
            f"{_export_dump_excluding_session_vars(chr(34) + '$__hermes_snap_tmp' + chr(34))}\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # truncated/half-written file. Use mktemp rather than $BASHPID: macOS
        # bash 3.2 leaves $BASHPID unset, while $$ is the parent shell PID and
        ...
        # grep pipeline segment. See _export_dump_excluding_session_vars.
        if self._snapshot_ready:
            parts.append(
                f"__hermes_snap_tmp=$(mktemp {_snap_tmp_template}) && "
                f"{{ {_export_dump_excluding_session_vars(chr(34) + '$__hermes_snap_tmp' + chr(34))} "
                f"&& mv -f \"$__hermes_snap_tmp\" {_quoted_snap}; }} "
                f"2>/dev/null || {{ [ -n \"${{__hermes_snap_tmp:-}}\" ] "
