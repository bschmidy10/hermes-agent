...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_mktemp_not_dollardollar(self):
        """The temp name MUST be generated by ``mktemp``, not ``$$`` or
        ``$BASHPID``.  On macOS bash 3.2 ``$BASHPID`` is unset in subshells,
        while ``$$`` stays the parent shell's PID, so both forms can collide
        across concurrent writers and publish a torn snapshot."""
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
        assert "mktemp " in wrapped
        assert ".tmp.XXXXXX" in wrapped
        # The colliding PID-derived temp forms must be gone.
        assert ".tmp.$$" not in wrapped
        assert "$BASHPID" not in wrapped

    def test_temp_path_static_part_is_quoted_mktemp_template(self):
        """The mktemp template's static path portion must be shlex-quoted so
        Windows/Git-Bash-like paths or spaces remain a single shell word."""
        env = _TestableEnv()
        env._snapshot_ready = True
        env._snapshot_path = "/tmp/has space/hermes-snap-x.sh"
        wrapped = env._wrap_command("echo hi", "/tmp")
        assert "mktemp '/tmp/has space/hermes-snap-x.sh.tmp.XXXXXX'" in wrapped
        # The space must never appear bare/unquoted in the temp template.
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH.  Crucially it uses ``mktemp`` for a portable
    per-writer unique file; PID-derived temp names collide on macOS bash 3.2.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        writer = (
            ...
            "__hermes_snap_tmp=$(mktemp " + _snap_tmp_template + ") && "
            f"{{ export -p > \"$__hermes_snap_tmp\" && mv -f \"$__hermes_snap_tmp\" {_q(snap)}; }} "
            "2>/dev/null || { [ -n \"${__hermes_snap_tmp:-}\" ] && rm -f \"$__hermes_snap_tmp\" 2>/dev/null || true; } "
            "done"
        )
